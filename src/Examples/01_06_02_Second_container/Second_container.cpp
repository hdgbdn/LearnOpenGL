...
const string APP_NAME = "Second container";
...
int main(int, char**)
{
	...
	glm::vec3 transform(.0f, .0f, .0f);
	glm::vec3 transform2(1.0f, 1.0f, .0f);
	...
	window.PushRenderOperation(Operation([&]
		{
			...
			trans = glm::translate(glm::mat4(1.0f), transform);
			trans = glm::rotate(trans, glm::radians(rotate_angle), rotate_axis);
			...
			trans = glm::translate(glm::mat4(1.0f), transform2);
			trans = glm::rotate(trans, glm::radians(rotate_angle), rotate_axis);
			shader.set("ratio", ratio);
			shader.set("transform", trans);
			ebo1.Draw();
			...
			ImGui::NewFrame();
			{
				...
				ImGui::SliderFloat3("Transform", reinterpret_cast<float*>(&transform), -1.f, 1.f);
				ImGui::SliderFloat3("Transform2", reinterpret_cast<float*>(&transform2), -1.f, 1.f);
				ImGui::End();
			}
