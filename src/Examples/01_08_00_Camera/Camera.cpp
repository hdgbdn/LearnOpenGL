...
int main(int, char**)
{
    ...
    EventMgr::GetInstance()->SetKeyCb(GLFW_KEY_W, ENVENT_TYPE::PRESSS, new LambdaOp([&](){
        cam.Move(Camera_Movement::FORWARD, EventMgr::GetInstance()->delta_time);
    }));

    EventMgr::GetInstance()->SetKeyCb(GLFW_KEY_S, ENVENT_TYPE::PRESSS, new LambdaOp([&](){
        cam.Move(Camera_Movement::BACKWARD, EventMgr::GetInstance()->delta_time);
    }));

    EventMgr::GetInstance()->SetKeyCb(GLFW_KEY_A, ENVENT_TYPE::PRESSS, new LambdaOp([&](){
        cam.Move(Camera_Movement::LEFT, EventMgr::GetInstance()->delta_time);
    }));

    EventMgr::GetInstance()->SetKeyCb(GLFW_KEY_D, ENVENT_TYPE::PRESSS, new LambdaOp([&](){
        cam.Move(Camera_Movement::RIGHT, EventMgr::GetInstance()->delta_time);
    }));

    EventMgr::GetInstance()->SetKeyCb(GLFW_MOUSE_BUTTON_RIGHT, ENVENT_TYPE::PRESSS, new LambdaOp([&](){
        cam.ViewMode = true;
    }));

    EventMgr::GetInstance()->SetKeyCb(GLFW_MOUSE_BUTTON_RIGHT, ENVENT_TYPE::RELEASE, new LambdaOp([&](){
        cam.ViewMode = false;
    }));

    EventMgr::GetInstance()->SetMousePosCb(new MousePosCb([&cam](double x, double y){
        cam.RotateByMouse(EventMgr::GetInstance()->mouse.delta.x, EventMgr::GetInstance()->mouse.delta.y);
    }));
    ...
    auto eventOp = new LambdaOp([&]()
        {
            glfwPollEvents();
            EventMgr::GetInstance()->UpdateFrame(glfwGetTime());
        });
