...
int main(int, char**)
{
	...
	Shader outlineShader((shader_path/"outline.vs").string(), (shader_path/"outline.fs").string());
	...
	glTexImage2D(GL_TEXTURE_2D, 0, GL_STENCIL_INDEX8, SCR_WIDTH, SCR_HEIGHT, 0, GL_STENCIL_INDEX, GL_INT, nullptr);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);
	glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);
	...
	window.PushRenderOperation(Operation([&]
		{
			...
			outlineShader.Use();
			glm::mat4 outlineModel = glm::scale(colorModel, glm::vec3(outlineScale, outlineScale, outlineScale));
			outlineShader.set("model", outlineModel);
			outlineShader.set("view", view);
			outlineShader.set("projection", projection);
			outlineShader.set("outlineColor", outlineColor);
			...
			ImGui::NewFrame();
			{
				...
				ImGui::Image(ImTextureID(colorBuffer), ImVec2(320, 180), ImVec2(0.0f, 1.0f), ImVec2(1.0f, 0.0f));
				ImGui::Image(ImTextureID(depthBuffer), ImVec2(320, 180), ImVec2(0.0f, 1.0f), ImVec2(1.0f, 0.0f));
				ImGui::Image(ImTextureID(stencilBuffer), ImVec2(320, 180), ImVec2(0.0f, 1.0f), ImVec2(1.0f, 0.0f));
				ImGui::EndTable();
				ImGui::End();
			}
